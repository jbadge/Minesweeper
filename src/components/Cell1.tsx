...
export function Cell({
  ...
  function handleClick(
    ...
    test(event)
    // check if game is over or bomb
    console.log(event.currentTarget)
    console.log(event.currentTarget.childNodes)
    console.log('DEBUG7')
  }

  // function test(e: React.MouseEvent, cell: string) {
  function test(e: React.MouseEvent) {
    // console.log(transformCell(cell))
    // console.log(e.currentTarget.children.item(0))
    // console.log(e.currentTarget.classList)
    if (cell === '*') {
      console.log('DEBUG8')
      // e.currentTarget.classList.add(`${transformCell(cell)}`)
